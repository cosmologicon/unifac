...
_mousevisible = True
def addcursor(surf):
    if drawmouse and _mousevisible:
        img, (hx, hy) = getcursorimg()
        px, py = _mpos if _mpos is not None else pygame.mouse.get_pos()
...
if __name__ == "__main__":
    ...
    fixedfps = True
    remakeaudio = True
    rawvideo = True   # Much larger video size, but YouTube converts it correctly
...
#    audioarr.tofile(open(rawfile, "wb"))
    ...
    com = "oggenc --raw --quiet -o %s %s" % (oggfile, rawfile)
    print com
    os.system(com)
